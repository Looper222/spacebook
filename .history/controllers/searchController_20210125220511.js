...
const { pullOutValues, geenerateArray } = require('../middleware/searchMiddleware');
...
const search_user = async (req, res, next) => {
    ...
    try {
        ...
        //     // console.log(element);
        //     res.json({ id: e._id, fname: e.fname, surname: e.surname, race: e.race, sex: e.sex, planet: e.planet });
        // });



        res.status(201).json({
            searchResults: pullOutValues(user)
            // onlyWanted: (user) => {
            //     user.forEach(e => {
