...
const handleErrorsSignup = (err) => {
    ...
    if (err.code === 11000) {
        ...
        //     return errors;
        // }
        if (err.message.includes('E11000 duplicate key error collection: spacebook.users index: email_1 dup key:')) {
            console.log('yes');
            errors.email = 'That email is already registered';
            return errors;
        }
        if (err.message.includes('E11000 duplicate key error collection: spacebook.users index: phoneNumber_1 dup key:')) {
            // console.log('yes');
            errors.phoneNumber = 'That number is already registered';
            return errors;
        }
        // if (error.message.includes('E11000 duplicate key error collection: spacebook.users index: phoneNumber_1 dup key:')) {
